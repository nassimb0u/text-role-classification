...
def main():
    ...
    if config["combine_type"] is None:
        ...
        chimer = {
            "train": load_from_disk(f"{chimer_path}/train"),
    ...
    if config["combine_type"] is None:
        if config["eval_mode"]:
            train_dataset, train_dataset_orig, eval_dataset, test_dataset = (
                make_train_eval_test_ds(ds, column_names, features, eval=True)
            )

        elif (
            config["transforms_augmentations"] is None
            and config["transforms_balancing"] is None
        ):
            train_dataset, train_dataset_orig, test_dataset = make_train_eval_test_ds(
                ds, column_names, features
            )
        ...
        if (
            ...
            train_dataset, train_dataset_orig, test_dataset = make_train_eval_test_ds(
                ds, column_names, features, combine_type=config["combine_type"]
            )
        else:
            train_dataset, train_dataset_orig, test_dataset = make_train_eval_test_ds(
                ds,
    ...
    for learning_rate in config["learning_rate"]:
        ...
        processor = AutoProcessor.from_pretrained(
            config["pretrained_model"], apply_ocr=False
        )
        ...
        if (
            config["transforms_balancing"] is not None
            and "weighted_loss" in config["transforms_balancing"]
        ):
        ...
        trainer.train(
            resume_from_checkpoint="/kaggle/working/LayoutLMv3/outputs/layoutlmv3_base_linear_None_False_10000_2_16_2e-05_noaug_nobal/checkpoint-10000"
        )
